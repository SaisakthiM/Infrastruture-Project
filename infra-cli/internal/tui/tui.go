...
var mainMenuItems = []menuItem{
	...
	{"  Update", "Check latest release, diff, and apply changed infra files"},
	{"  Exit", "Quit the TUI"},
}
	...
	case tea.KeyMsg:
		...
		case screenMain:
			...
			case "enter", " ":
				...
				// Import State and Update don't need env selection
				if m.mainCursor == 4 || m.mainCursor == 5 {
...
func (m model) runOperation(opIdx int, env string) error {
	...
	case 5: // Update
		...
		fmt.Printf("  Latest: %s   Installed: %s\n", rel.TagName, m.cfg.ReleaseTag)

		if rel.TagName == m.cfg.ReleaseTag {
			fmt.Println("  ✓ Already up to date.")
			return nil
		}

		fmt.Println("  Downloading new release for comparison...")
		...
		fmt.Println("  Diffing against installed infra...")
		changes, err := release.DiffInfra(newInfraRoot, m.cfg.InfraDir)
		if err != nil {
			return err
		}

		if len(changes) == 0 {
			fmt.Println("  No file differences — already in sync.")
			m.cfg.ReleaseTag = rel.TagName
		...
		fmt.Printf("\n  %d added, %d modified, %d removed (tfvars/tfstate/.terraform untouched)\n", added, modified, removed)

		fmt.Printf("\n  Apply these %d change(s) and update to %s? [y/N]: ", len(changes), rel.TagName)
		var ans string
		fmt.Scanln(&ans)
		if strings.ToLower(ans) != "y" {
			fmt.Println("  Aborted.")
		...
		if err := release.ApplyUpdate(changes, newInfraRoot, m.cfg.InfraDir); err != nil {
			return err
		}
		m.cfg.ReleaseTag = rel.TagName
		...
		fmt.Printf("  ✓ Updated to %s (%d file(s) changed)\n", rel.TagName, len(changes))
		return nil
	}
	return nil
}
